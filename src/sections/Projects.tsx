              ...
              >
                <TiltCard maxTilt={8} glareOpacity={0.3} className="h-full">
                  <div
                    ...
                    className="project-card-interactive group flex flex-col h-full rounded-3xl glass-card-3d border border-white/10 hover:border-primary-500/50 hover:shadow-2xl hover:shadow-primary-500/15 transition-all duration-300 overflow-hidden cursor-pointer"
                  >
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center p-4 backdrop-blur-sm z-20">
                        <span className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider bg-gradient-to-r from-primary-600 to-accent-cyan text-white shadow-xl">
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                      ...
                      {project.metrics && project.metrics.length > 0 && (
                        <div className="flex items-center gap-3 pt-3 border-t border-slate-800/80 dark:border-slate-800/80 light:border-slate-200 text-xs">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {project.technologies.slice(0, 4).map((tech) => (
                          <span
                            key={tech}
                            className="px-2.5 py-0.5 rounded-lg text-[11px] font-mono bg-slate-800/90 dark:bg-slate-800/90 light:bg-slate-100 text-slate-300 dark:text-slate-300 light:text-slate-700 border border-slate-700/60 dark:border-slate-700/60 light:border-slate-300"
                          >
                            {tech}
                          </span>
                        ))}
                        {project.technologies.length > 4 && (
                          <span className="px-2 py-0.5 rounded-lg text-[11px] font-mono text-slate-400">
                            +{project.technologies.length - 4}
                          </span>
                        )}
                      </div>
                      ...
                      {/* Actions Footer */}
                      <div
                        className="pt-4 border-t border-slate-800/80 dark:border-slate-800/80 light:border-slate-200 flex items-center justify-between gap-2"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <button
                          onClick={() => setActiveProject(project)}
                          className="text-xs font-semibold text-primary-400 hover:text-primary-300 flex items-center gap-1.5 transition-colors cursor-pointer"
                        >
                        ...
                        <div className="flex items-center gap-2">
                          {project.githubUrl && (
                            <a
                              ...
                              className="p-2 rounded-xl bg-slate-800/90 hover:bg-slate-700 text-slate-300 hover:text-white border border-slate-700/60 transition-all hover:scale-105"
                          ...
                          {project.liveUrl && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-2 rounded-xl bg-primary-600/20 hover:bg-primary-600/30 text-primary-300 hover:text-white border border-primary-500/40 transition-all hover:scale-105"
