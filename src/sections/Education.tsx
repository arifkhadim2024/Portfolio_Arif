          ...
          {educationData.map((item, index) => (
            ...
            >
              <TiltCard maxTilt={6} glareOpacity={0.2}>
                <div className="glass-card-3d p-6 sm:p-8 rounded-3xl space-y-5 border border-white/10 hover:border-primary-500/40 hover:shadow-xl transition-all">
                  {/* Degree & Institution */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800 dark:border-slate-800 light:border-slate-200">
                    ...
                    <div className="flex flex-col sm:items-end gap-2">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono font-semibold bg-slate-800/90 dark:bg-slate-800/90 light:bg-slate-100 text-slate-300 dark:text-slate-300 light:text-slate-700 border border-slate-700/60 dark:border-slate-700/60 light:border-slate-300 w-fit">
                        <Calendar className="w-3.5 h-3.5 text-accent-cyan" />
                        {item.startDate} — {item.endDate}
                      </span>
                      {item.grade && (
                        <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/30 w-fit shadow-sm">
